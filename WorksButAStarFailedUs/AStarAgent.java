...
public class AStarAgent extends Agent {
    ...
    {
    	return state.getUnit(enemyFootmanID) != null && currentPath.size() > 0 && currentPath.peek().x == state.getUnit(enemyFootmanID).getXPosition() && 
    ...
    {
    	//start at the bottom left of the neighbors
    	int currentposx = goal.x - 1; 
        ...
        path.clear(); // used during recalculate path to ensure footman doesn't looks at new path
    	...
    	while(!(cheapest.x == start.x && cheapest.y == start.y)) 
    	{
    		...
    		if(cheapest.x != start.x || cheapest.y != start.y) //makes sure path doesn't push start or goal location
    		{
    			path.push(cheapest);
    			cheb_prev= (int) Double.POSITIVE_INFINITY; // resets cheb value
    			
    			closed.put(cheapest.x + cheapest.y, cheapest); // marks coordinate as closed so won't relook at it
